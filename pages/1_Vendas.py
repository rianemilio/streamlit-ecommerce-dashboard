...
category_translation_raw = pd.Series(translation_df.product_category_name.values, index=translation_df.product_category_name_english).to_dict()
category_translation = { en: pt.replace('_', ' ').title() for en, pt in category_translation_raw.items() }
category_translation['unknown'] = 'Desconhecida'
payment_type_translation = { 'credit_card': 'Cartão de Crédito', 'boleto': 'Boleto', 'voucher': 'Voucher', 'debit_card': 'Cartão de Débito', 'not_defined': 'Não Definido' }
...
if not df_filtered.empty:
    ...
    with col1:
        ...
        fig_monthly = px.area(monthly_revenue, x='order_purchase_timestamp', y='price', height=225)
    ...
    with col2:
        ...
        fig_cat = px.bar(revenue_by_category, x='price', y=revenue_by_category.index, orientation='h', text_auto='.2s', height=225)
    ...
    with col3:
        ...
        fig_payment = px.pie(payment_distribution, names=payment_distribution.index, values='payment_value', hole=0.5, height=225)
    ...
    with col4:
        ...
        fig_state = px.bar(revenue_by_state, x=revenue_by_state.index, y='price', text_auto='.2s', height=225)
