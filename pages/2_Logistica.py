...
def load_data():
    data_path = 'data/'
    try:
        cols_orders = ['order_id', 'customer_id', 'order_purchase_timestamp', 'order_delivered_customer_date', 'order_estimated_delivery_date']
    ...
    df.dropna(subset=['order_purchase_timestamp', 'order_delivered_customer_date', 'order_estimated_delivery_date'], inplace=True)
...
def process_logistics_data(df):
    ...
    df['estimated_time'] = (df['order_estimated_delivery_date'] - df['order_purchase_timestamp']).dt.days
    df['delivery_delay'] = (df['order_delivered_customer_date'] - df['order_estimated_delivery_date']).dt.days

    df = df[df['delivery_time'] >= 0]
    
    df['delivery_status'] = df['delivery_delay'].apply(lambda x: 'Atrasado' if x > 0 else 'No Prazo')
...
st.sidebar.header("Filtros de Logística")
min_date_log = df_processed['order_purchase_timestamp'].min().date()
max_date_log = df_processed['order_purchase_timestamp'].max().date()
...
if not df_filtered_log.empty:
    ...
    delay_percentage = (df_filtered_log['delivery_status'] == 'Atrasado').sum() / len(df_filtered_log) * 100
    ...
    with col1:
        st.markdown('<p class="chart-title">Performance de Entrega</p>', unsafe_allow_html=True)
        status_count = df_filtered_log['delivery_status'].value_counts()
        pull_values = [0.1 if label == 'Atrasado' else 0 for label in status_count.index]
        fig_pie = px.pie(
            status_count, 
            values=status_count.values, 
            names=status_count.index,
            hole=0.5,
            color_discrete_map={'No Prazo': '#2ca02c', 'Atrasado': '#d62728'},
            height=250
        )
    ...
    with col2:
        st.markdown('<p class="chart-title">Tempo Médio de Entrega (Mensal)</p>', unsafe_allow_html=True)
        monthly_delivery_time = df_filtered_log.set_index('order_purchase_timestamp').resample('ME')['delivery_time'].mean().reset_index()
        fig_line = px.line(
        ...
        fig_line.update_layout(margin=dict(l=10, r=10, t=20, b=20), yaxis_title="Dias", xaxis_title=None)
        st.plotly_chart(fig_line, use_container_width=True)

    state_performance = df_filtered_log.groupby('customer_state', observed=False).agg(
        avg_delivery_time=('delivery_time', 'mean'),
        delay_percentage=('delivery_status', lambda x: (x == 'Atrasado').sum() / len(x) * 100)
    ).reset_index()

    col3, col4 = st.columns(2)
    with col3:
        st.markdown('<p class="chart-title">Top 10 Estados (Maior Tempo)</p>', unsafe_allow_html=True)
        ...
        fig_bar_time.update_layout(margin=dict(l=10, r=10, t=20, b=20), xaxis_title="Dias", yaxis_title=None)
        st.plotly_chart(fig_bar_time, use_container_width=True)

    with col4:
        st.markdown('<p class="chart-title">Top 10 Estados (Maior % Atraso)</p>', unsafe_allow_html=True)
        ...
        fig_bar_delay.update_layout(margin=dict(l=10, r=10, t=20, b=20), xaxis_title="%", yaxis_title=None)
